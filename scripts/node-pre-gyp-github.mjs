...
// Local fork of https://github.com/bchr02/node-pre-gyp-github
...
export default class NodePreGypGithub {
  ...
  init() {
    ...
    if (!this.package_json.repository || !this.package_json.repository.url) {
      ...
      const ownerRepo = this.package_json.repository.url.match(
        /https?:\/\/([^\/]+)\/(.*)(?=\.git)/i
      );
      if (!ownerRepo) {
        throw new Error(
    ...
    if (
      ...
      throw new Error(
        'binary.host in package.json should begin with: "' + hostPrefix + '"'
      );
    }
  ...
  createOctokitInstance(token) {
    return new Octokit({
      ...
      auth: token,
      userAgent: this.package_json.name
        ? this.package_json.name
        : 'node-pre-gyp-github',
    });
  }
  ...
  async uploadAssets() {
    ...
    if (!files.length)
      throw new Error(
        'No files found within the stage directory: ' + this.stage_dir
      );

    for (const file of files) {
      if (this.release && this.release.assets) {
        const asset = this.release.assets.filter(
  ...
  async publish(options = {}) {
    verbose =
      typeof options.verbose === 'undefined' || options.verbose ? true : false;
    ...
    const release = data.filter(
      (element) => element.tag_name === options.tag_name
    );
    ...
    if (release.length === 0) {
      ...
      if (this.release.draft) {
        consoleLog(
          `Release ${this.release.tag_name} not found, so a draft release was created. YOU MUST MANUALLY PUBLISH THIS DRAFT WITHIN GITHUB FOR IT TO BE ACCESSIBLE.`
        );
      } else {
        consoleLog(
          `Release ${this.release.tag_name} not found, so a new release was created and published.`
        );
      }
    } else {
